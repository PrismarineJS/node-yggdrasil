...
var Server = function (options) {
  this.host = options.host || 'https://sessionserver.mojang.com'
}
 ...
 * @async
 */
Server.prototype.join = function (token, profile, serverid, sharedsecret, serverkey, cb) {
  utils.call(this.host, 'session/minecraft/join', {
    'accessToken': token,
 ...
 * @async
 */
Server.prototype.hasJoined = function (username, serverid, sharedsecret, serverkey, cb) {
  var hash = utils.mcHexDigest(crypto.createHash('sha1').update(serverid).update(sharedsecret).update(serverkey).digest())
  utils.request({
    method: 'GET',
    uri: this.host + '/session/minecraft/hasJoined?username=' + username + '&serverId=' + hash,
    json: true
